...
bind_kag_17 <- kaggle_17_pl %>%
  ...
  mutate(Źródło = "Kaggle 2017", wiek = as.character(wiek))
# kaggle 2018
...
bind_kag_18 <- kaggle_18_pl %>% 
  ...
  mutate(Źródło = "Kaggle 2018", wiek = as.character(wiek))
...
bind_so_18 <- so_18_pl %>% 
  ...
  mutate(Źródło = "Stack Overflow 2018", wiek = as.character(wiek))
...
bind_so_19 <- so_19_pl %>%
  ...
  rename(płeć = Gender, wiek = Age, poziom_wykształcenia = EdLevel,
         kierunek_wykształcenia = UndergradMajor) %>% 
  mutate(Źródło = "Stack Overflow 2019", wiek = as.character(wiek))
...
bind_whyr_17 <- whyr_17_pl %>%
  ...
  rename(poziom_wykształcenia = `Stopień naukowy do umieszczenia na certyfikacie uczestnictwa w konferencji (o ile dotyczy)`,
         płeć = Płeć) %>% 
  mutate(Źródło = "WhyR? 2017", wiek = as.character(wiek))

# pydata 2018
bind_py_18 <- pydata_18_pl %>% select(Gender) %>% rename(płeć = Gender) %>% 
  mutate(Źródło = "PyData 2018")

# bind and write file
d <- bind_rows(bind_kag_17, bind_kag_18, bind_so_18, bind_so_19, 
               bind_whyr_17, bind_py_18)
summary(factor(d$Źródło))

write_csv(d, "demographics.csv")
